...
with open(csv_filename_CV, mode="w", newline="") as file:
    ...
    writer.writerow(["Set", "Feature Group", "Accuracy", "Precision", "Recall", "F1-score", "n_estimators", "max_depth", "learning_rate", "subsample", "min_child_weight"])
...
for iteration_index, selected_groups in enumerate(feature_sets):
    ...
    for i in range(1, 6):
        ...
        # Grid Search
        gridsearchmodel = xgb.XGBClassifier(eval_metric='logloss', objective='binary:logistic', tree_method='hist', random_state=0)
        param_grid = {
        ...
        # Create ML model
        model = xgb.XGBClassifier(max_depth=grid_search.best_params_['max_depth'],
                                  n_estimators=grid_search.best_params_['n_estimators'],
                                  learning_rate=grid_search.best_params_['learning_rate'],
                                  subsample=grid_search.best_params_['subsample'],
                                  min_child_weight=grid_search.best_params_['min_child_weight'], random_state=0)
